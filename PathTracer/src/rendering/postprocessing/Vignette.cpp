...
namespace RT
{
	...
	*/
	...
	Vignette::Vignette(ID3D12Device* device, settings_struct* settings): PostProcessing(device, settings, 1.0F)
	{
		mNeedsInput = false;

		init(device, { L"vignette" });

		VignetteConfig config = { 1.0F / settings->width, 1.0F / settings->height };
		passCB = std::make_unique<UploadBuffer<VignetteConfig>>(device, 1, true);
		passCB->copyData(0, config);
	}

	void Vignette::effect(UINT index, ID3D12Resource* backBuffer, ID3D12Resource* copyTo)
	{
		...
		barriers[1] = CD3DX12_RESOURCE_BARRIER::Transition(backBuffer, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PRESENT);
		mCommandList[index]->ResourceBarrier(2, barriers);
	}

	void Vignette::buildRootSignature(ID3D12Device* device)
	{
		CD3DX12_DESCRIPTOR_RANGE uavRange;
		uavRange.Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 0, 0, 1);

		CD3DX12_ROOT_PARAMETER slotRootParameter[2];
		slotRootParameter[0].InitAsConstantBufferView(0);
		slotRootParameter[1].InitAsDescriptorTable(1, &uavRange, D3D12_SHADER_VISIBILITY_ALL);
